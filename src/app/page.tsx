...
const PageContent = () => {
  ...
  return (
    <div className="container min-h-screen m-auto">
      ...
      <section className={`flex-1 h-screen p-6 flex justify-center ${theme === 'light' ? 'bg-gradient-to-r from-teal-700 to-emerald-400' : 'bg-gradient-to-r from-teal-950 to-gray-950'}`} >
      
        <ToDoList />
        <WeatherSideBar/>
      </section>

      <section className={`flex-1 min-h-screen flex md:flex-row items-center px-6 relative ${theme === 'light' ? 'bg-white text-black' : 'bg-zinc-900 text-white'}`}>
      ...
      <section className={`flex-1 min-h-screen flex flex-col md:flex-row  justify-center items-center p-6 ${theme === 'light' ? 'bg-teal-600' : 'bg-teal-950'} text-white`}>
